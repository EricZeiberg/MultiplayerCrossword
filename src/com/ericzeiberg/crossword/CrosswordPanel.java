...
public class CrosswordPanel extends JPanel {
    ...
    public CrosswordPanel(Crossword puzzle, MainFrame mainFrame) {
        this.mainFrame = mainFrame;
        this.puzzle = puzzle.getCells();
                ...
                switch (e.getKeyCode()) {
                    ...
                    default:
                        ...
                        if (!cellToHighlight.equals(new Point(-1, -1))) {
                            ...
                            if (c >= 'A' && c <= 'Z')
                                setCell(Character.toUpperCase(e.getKeyChar()), true);
                        }
                        break;
                }
            }
    ...
    private void drawCell(Cell cell, int x, int y, Graphics2D g) {
        ...
        if (cell == null) {
            ...
            if (cellToHighlight.equals(new Point(x, y))) // cell clicked on
                ...
                // clue clicked on
            else if ((cell.hasAcross() && cell.getAcross().getId() == clueToHighlight && highlightDirection == ACROSS)
                    || (cell.hasDown() && cell.getDown().getId() == clueToHighlight && highlightDirection == DOWN))
        ...
        if (cell != null) {
            ...
            if (this.cellToHighlight.equals(cellHighlight)) { // clicked same
                ...
                if (highlightDirection == ACROSS) {
                    if (cell.hasDown()) {
                        highlightClue(cell.getDown(), DOWN);
                ...
                if (cell.hasAcross()) {
                    ...
                    // highlight down clue
                    // and not across
                    if (cell.hasDown() && cell.getDown().getId() == clueToHighlight) {
                        highlightClue(cell.getDown(), DOWN);
                        this.cellToHighlight = cellHighlight;
                    } else {
                        highlightClue(cell.getAcross(), ACROSS);
                        this.cellToHighlight = cellHighlight;
                    }
     ...
     */
    private void highlightClue(Entry clue, int direction) {
        highlightDirection = direction;
        if (direction == NONE) {
            clueToHighlight = 0;
        } else {
            clueToHighlight = clue.getId();
        }
    ...
    protected void onlyHighlightClue(int x, int y, int clueNum, int direction) {
        if (direction == ACROSS) {
            highlightClue(puzzle[x][y].getAcross(), direction);
        } else {
            highlightClue(puzzle[x][y].getDown(), direction);
        }
     ...
     */
    private void selectClueInList(Entry clue, int direction) {
        if (clue != null) {
            if (direction == ACROSS) {
                mainFrame.getDownClues().clearSelection();
                mainFrame.getAcrossClues().setSelectedValue(clue, true);
            } else if (direction == DOWN) {
                mainFrame.getAcrossClues().clearSelection();
                mainFrame.getDownClues().setSelectedValue(clue, true);
            }
        } else {
            mainFrame.getAcrossClues().clearSelection();
            mainFrame.getDownClues().clearSelection();
     ...
     */
    protected void setCell(int x, int y, char character) {
        setCell(x, y, character, true, true);
    }
    ...
    protected void setCell(char character, boolean forward) {
        setCell(cellToHighlight.x, cellToHighlight.y, character, forward, true);
    }
     ...
     */
    protected void setCell(int x, int y, char character, boolean forward,
                           boolean move) {
        int dir = NONE;
        if (move) {
            if (forward) {
     ...
     *            - direction to move
     */
    protected void setCell(int x, int y, char c, int direction) {
        puzzle[x][y].setC(Character.toString(c));
        checkClueSolved(puzzle[x][y]);
        move(direction);
    }
    ...
    private void checkClueSolved(Cell cell) {
        if (cell.hasAcross())
            checkClueSolved(cell.getAcross(), ACROSS);
        if (cell.hasDown())
            checkClueSolved(cell.getDown(), DOWN);
     ...
     *            - username of person who edited cell
     */
    private void checkClueSolved(Entry clue, int direct) {
        boolean solved = true;
        String direction;
        if (direct == ACROSS)
            direction = "across";
        ...
        if (direct == ACROSS) { // check across
            int y = clue.getY();
            for (int x = clue.getX(); x < clue.getX() + clue.getLength(); x++) {
            ...
            int x = clue.getX();
            for (int y = clue.getY(); y < clue.getY() + clue.getLength(); y++) {
                solved = solved && puzzle[x][y].getC().equals(puzzle[x][y].getAnswer());
                if (!solved) // clue not solved, stop checking
                    break;
            }
